...
class Validator:
    ...
    def create_menu_message(self):
        if self.create_menu() == "dateFormatError":
            ...
            return jsonify(
                {"message": "The date you provided already has a menu"}), 422
    ...
    def create_order_message(self):
        if self.create_order() == "customer_idError":
            ...
            return jsonify(
                {"message": "Date should be an integer, not string"}), 422
            ...
            return jsonify(
                {"message": "Date with id "+str(self.data['date']) + " not found"}), 422
        elif self.create_order() == "noMealInMenuError":
            return jsonify(
                {"message": "The meal provided is not in the menu for date with id " + str(self.data['date'])}), 422
